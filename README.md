...
安装和 API Key 配置完成后，运行 `omd setup` 进入交互式安装向导，选择适合你的使用模式：

| 模式 | 适用场景 | 效果 |
|------|---------|------|
| **① 独立模式** | 直接终端使用，无需编码智能体 | 配置 API Key → 验证连接 → 可用 `omd run` / `omd chat` |
| **② 在编码智能体中运行 OMD** | Claude Code / Codex 用户，希望智能体本身跑在 DeepSeek 上 | 自动配置智能体的 API 提供商为 DeepSeek + 注册 OMD 为 MCP |
| **③ MCP 模式** | 已有编码智能体，仅需 OMD 作为工具增强 | 注册 OMD 为 MCP 服务器，不改智能体原有 API 配置 |
| **④ 手动配置** | 自行控制配置细节，或使用非自动检测的 MCP 客户端 | 显示适用于各客户端的 JSON 配置模板 |

> **快速选择**：日常终端使用选 ①；想 Claude Code / Codex 用 DeepSeek + OMD 多智能体能力选 ②；已有智能体配置不想动、只加 OMD 工具选 ③。
